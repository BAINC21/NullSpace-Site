...
// ─── Donate Panel ───
function DonatePanel({ open, onClose }) {
      ...
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 460 }}>
        ...
        <div className="modal-body">
          ...
          <div className="crypto-address-box">
            <code>{DONATE_CONFIG.btcAddress}</code>
            <button onClick={() => copy(DONATE_CONFIG.btcAddress, "btc")}>
          ...
          <div className="crypto-address-box">
            <code>{DONATE_CONFIG.xmrAddress}</code>
            <button onClick={() => copy(DONATE_CONFIG.xmrAddress, "xmr")}>
...
// ─── Add Media Modal (admin) ───
function MediaModal({ open, onClose, onSave }) {
...
// ─── HOME PAGE (Directory) ───
function HomePage({ projects, media, onNavigate, isLoggedIn, onAddMedia, onDeleteMedia }) {
  ...
  return (
    <div className="fade-in">
      ...
      <DonatePanel open={donateOpen} onClose={() => setDonateOpen(false)} />
...
// ─── ADMIN PAGE ───
function AdminPage({ projects, setProjects, analytics, isLoggedIn, onLogin, inboxMessages, onClearInbox, onSaveMessage, onDisregardMessage, onReplyToMessage, credentials, onUpdateCredentials, onSaveProject, onDeleteProject, onRestoreProject, onPermanentDeleteProject, connStatus, activityLog }) {
          ...
          {inboxMessages.length === 0 ? (
            ...
            inboxMessages.map((m) => (
              <div key={m.id} className="admin-project-row" style={{ flexDirection: "column", alignItems: "flex-start", gap: 8, opacity: m.status === "disregarded" ? 0.4 : 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, width: "100%" }}>
                  ...
                  {m.status !== "disregarded" && (
                    <div className="msg-actions">
                      {/* Reply — email or inline */}
                      {m.email ? (
                        <a href={`mailto:${m.email}?subject=Re: Your message to NullSpace Studio&body=Hi,%0A%0AReplying to your message: "${m.text}"%0A%0A`} className="btn-green" style={{ fontSize: 11, padding: "4px 10px", textDecoration: "none" }}>
                          Reply
                        </a>
                      ) : (
                        <button className="btn-green" style={{ fontSize: 11, padding: "4px 10px" }} onClick={() => { setReplyingTo(replyingTo === m.id ? null : m.id); setReplyText(""); }}>
                          {replyingTo === m.id ? "Cancel" : "Reply to Q&A"}
                        </button>
                      )}
                      {/* Save */}
                      {m.status !== "saved" && (
                ...
                {/* Inline reply form for no-email messages */}
                {replyingTo === m.id && !m.email && (
                  <div className="inline-reply-box">
                    <input
                      value={replyText}
                      onChange={e => setReplyText(e.target.value)}
                      placeholder="Type your reply (will be posted to Q&A)..."
                      onKeyDown={e => { if (e.key === "Enter" && replyText.trim()) { onReplyToMessage(m, replyText.trim()); setReplyingTo(null); setReplyText(""); } }}
                      autoFocus
                    />
                    <button className="btn-green" style={{ fontSize: 12, padding: "8px 14px" }} onClick={() => { if (replyText.trim()) { onReplyToMessage(m, replyText.trim()); setReplyingTo(null); setReplyText(""); } }}>
                      Post to Q&A
                    </button>
                  </div>
                )}
        ...
        <div className="admin-table-wrap" style={{ maxWidth: 560 }}>
          ...
          <div style={{ padding: 24 }}>
            <p style={{ fontSize: 13, color: "var(--text-dim)", marginBottom: 24, lineHeight: 1.6 }}>
              Update your passkey, email, or password below. Changes take effect immediately.
            </p>
            ...
            <div className="field">
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  type="password" value={newPasskey}
                  onChange={e => setNewPasskey(e.target.value)}
                  placeholder="Enter new passkey"
                  style={{ flex: 1 }}
                />
                <button className="btn-green" onClick={() => handleCredUpdate("passkey")}>
                  {credSaved === "passkey" ? "Saved!" : "Update"}
                </button>
              </div>
            </div>
            ...
            <div className="field">
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  ...
                  style={{ flex: 1 }}
                />
                <button className="btn-green" onClick={() => handleCredUpdate("email")}>
                  {credSaved === "email" ? "Saved!" : "Update"}
                </button>
              </div>
            </div>
            ...
            <div className="field">
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  ...
                  style={{ flex: 1 }}
                />
                <button className="btn-green" onClick={() => handleCredUpdate("password")}>
                  {credSaved === "password" ? "Saved!" : "Update"}
                </button>
              </div>
            </div>

            <div style={{ marginTop: 24, padding: 16, background: "var(--black-5)", borderRadius: "var(--radius-lg)", border: "1px solid var(--border)" }}>
              <p style={{ fontSize: 12, fontFamily: "var(--font-mono)", color: "var(--text-muted)", lineHeight: 1.6 }}>
                Current email: <span style={{ color: "var(--text)" }}>{credentials.email}</span>
                <br />
                Passkey & password are hidden for security.
              </p>
            </div>
          </div>
        </div>
...
export default function App() {
      ...
      try {
        if (isSupabaseConfigured()) {
          ...
          if (connTest.ok) {
            ...
            const [p, m, msgs, creds, stats, log, qa] = await Promise.all([
              db.loadProjects(),
              db.loadMedia(),
              db.loadMessages(),
              db.loadSettings(),
  ...
  const handleReplyToMessage = async (msg, replyText) => {
    // Post to Q&A
    const qaPost = {
      id: uid(),
      question: msg.text,
      answer: replyText,
      asker_email: msg.email || "",
      created_at: new Date().toISOString(),
    };
