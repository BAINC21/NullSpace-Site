...
function AnalyticsChart({ data, range, onRangeChange }) {
    ...
    return (
      <div className="chart-container">
        ...
        <div className="empty-state" style={{ padding: "40px 20px" }}>
          <p>No traffic data yet. Visits will appear here as people view the site.</p>
        </div>
      </div>
    );
  }
  ...
  const linePoints = data.map((d, i) => {
    ...
    const y = 100 - (d.visitors / maxVal) * 85;
    return `${x},${y}`;
  }).join(" ");
        ...
        {data.map((d, i) => (
          <div key={i} className="chart-col" onMouseEnter={() => setHoveredIdx(i)} onMouseLeave={() => setHoveredIdx(null)}>
            <div className="chart-bar-stack">
              <div className="chart-bar visitors" style={{ height: `${(d.visitors / maxVal) * 85}%` }} />
              {d.clicks > 0 && <div className="chart-bar clicks" style={{ height: `${(d.clicks / maxVal) * 85}%` }} />}
            </div>
            {hoveredIdx === i && <div className="chart-tooltip">{d.day}: {d.visitors} views, {d.clicks} clicks</div>}
        ...
        <>
          ...
          {analytics.recentVisitors && analytics.recentVisitors.length > 0 && (
            <div className="recent-visitors">
              <div className="recent-visitors-header">Recent Visitors</div>
