...
const DEFAULT_DONATE = {
  buyMeACoffee: "https://buymeacoffee.com/nullspacestudio",
  btcAddress: "bc1qYOUR_BTC_ADDRESS_HERE",
...
const db = {
  ...
  async loadSiteConfig() {
    if (!isSupabaseConfigured()) return DEFAULT_DONATE;
    const { data, error } = await supabase.from("site_config").select("*").eq("id", 1).single();
    if (error || !data) return DEFAULT_DONATE;
    return {
      buyMeACoffee: data.buy_me_a_coffee || DEFAULT_DONATE.buyMeACoffee,
      btcAddress: data.btc_address || DEFAULT_DONATE.btcAddress,
      xmrAddress: data.xmr_address || DEFAULT_DONATE.xmrAddress,
  ...
  async updateSiteConfig(updates) {
    if (!isSupabaseConfigured()) return { ok: false };
    const row = {};
    if (updates.buyMeACoffee !== undefined) row.buy_me_a_coffee = updates.buyMeACoffee;
    if (updates.btcAddress !== undefined) row.btc_address = updates.btcAddress;
    if (updates.xmrAddress !== undefined) row.xmr_address = updates.xmrAddress;
  ...
  async trackVisit() {
    ...
    } catch (e) { console.error("Track visit:", e); }
  },
  async trackClick() {
    if (!isSupabaseConfigured()) return;
    try {
  ...
  async loadAnalytics() {
    if (!isSupabaseConfigured()) return { totalVisitors: 0, totalClicks: 0, uniqueVisitors: 0, bounceRate: 0, daily: [] };
    const { data, error } = await supabase.from("analytics").select("*").eq("id", 1).single();
    if (error || !data) return { totalVisitors: 0, totalClicks: 0, uniqueVisitors: 0, bounceRate: 0, daily: [] };
    return { totalVisitors: data.total_visitors || 0, totalClicks: data.total_clicks || 0, uniqueVisitors: data.unique_visitors || 0, bounceRate: 0, daily: [] };
  },
};
...
.chart-bar-pair {
  display: flex; gap: 4px; align-items: flex-end; width: 100%;
...
.chart-bar.visitors { background: var(--green); opacity: 0.7; }
.chart-bar.visitors:hover { opacity: 1; }
.chart-bar.clicks { background: var(--text-muted); }
.chart-bar.clicks:hover { background: var(--text-dim); }
.chart-bar-label {
  font-family: var(--font-mono); font-size: 11px; color: var(--text-muted);
}
    ...
    <div className="chart-container">
      <div className="chart-header">
        ...
        <div className="chart-legend">
          <span><div className="dot-green" /> Visitors</span>
          <span><div className="dot-dim" /> Clicks</span>
        </div>
      </div>
...
function DonatePanel({ open, onClose, donateConfig }) {
  ...
  const cfg = donateConfig || DEFAULT_DONATE;
      ...
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 460 }}>
        ...
        <div className="modal-body">
          ...
          <div className="crypto-address-box">
            <code>{cfg.btcAddress}</code>
            <button onClick={() => copy(cfg.btcAddress, "btc")}>
          ...
          <div className="crypto-address-box">
            <code>{cfg.xmrAddress}</code>
            <button onClick={() => copy(cfg.xmrAddress, "xmr")}>
              {copied === "xmr" ? "Copied!" : "Copy"}
            </button>
          </div>
        </div>
      </div>
...
// ─── HOME PAGE (Directory) ───
function HomePage({ projects, media, onNavigate, isLoggedIn, onAddMedia, onEditMedia, onDeleteMedia, donateConfig, maxMediaPosts }) {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="hero">
        ...
        <p>
          Every website, app, API, and tool built under NullSpace Studio — all in one place.
          Browse live projects or reach out to learn more.
        </p>
        {/* Donate Bar */}
                  ...
                  <div key={m.id} className="media-card" style={{ position: "relative" }}>
                    {isLoggedIn && (
                      <div style={{ position: "absolute", top: 8, right: 8, zIndex: 5, display: "flex", gap: 4 }}>
                        <button className="btn-ghost" style={{ fontSize: 10, padding: "3px 8px" }} onClick={() => onEditMedia(m)}>
                          Edit
                        </button>
                        <button className="btn-danger" style={{ fontSize: 10, padding: "3px 8px" }} onClick={() => onDeleteMedia(m.id)}>
                          {Icons.trash}
                        </button>
                      </div>
                    )}
...
// ─── ADMIN PAGE ───
function AdminPage({ projects, setProjects, analytics, isLoggedIn, onLogin, inboxMessages, onClearInbox, onSaveMessage, onDisregardMessage, onReplyToMessage, credentials, onUpdateCredentials, onSaveProject, onDeleteProject, onRestoreProject, onPermanentDeleteProject, connStatus, activityLog, siteConfig, onUpdateSiteConfig }) {
  ...
  // Donate/config state
  const [newCoffeeLink, setNewCoffeeLink] = useState("");
  const [newBtcAddress, setNewBtcAddress] = useState("");
  const [newXmrAddress, setNewXmrAddress] = useState("");
  ...
  const handleConfigUpdate = (field, value) => {
    ...
    const labelMap = { buyMeACoffee: "coffee", btcAddress: "btc", xmrAddress: "xmr", maxMediaPosts: "max" };
    setConfigSaved(labelMap[field] || field);
    setTimeout(() => setConfigSaved(""), 2000);
    if (field === "buyMeACoffee") setNewCoffeeLink("");
    if (field === "btcAddress") setNewBtcAddress("");
    if (field === "xmrAddress") setNewXmrAddress("");
  };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 560 }}>
          ...
          <div className="admin-table-wrap">
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ marginTop: 16, padding: 12, background: "var(--black-5)", borderRadius: "var(--radius)", border: "1px solid var(--border)" }}>
                <p style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--text-muted)" }}>
                  Current email: <span style={{ color: "var(--text)" }}>{credentials.email}</span>
                </p>
              </div>
            </div>
          </div>

          {/* Donate / Wallet Settings */}
          <div className="admin-table-wrap">
            <div className="admin-table-header">
              <h3>Donate & Wallet Settings</h3>
            </div>
            <div style={{ padding: 24 }}>
              <div className="field">
                <label>Buy Me a Coffee URL</label>
                <div style={{ display: "flex", gap: 8 }}>
                  <input value={newCoffeeLink} onChange={e => setNewCoffeeLink(e.target.value)} placeholder="https://buymeacoffee.com/..." style={{ flex: 1 }} />
                  <button className="btn-green" onClick={() => handleConfigUpdate("buyMeACoffee", newCoffeeLink)}>{configSaved === "coffee" ? "Saved!" : "Update"}</button>
                </div>
              </div>
              <div className="field">
                <label>Bitcoin (BTC) Address</label>
                <div style={{ display: "flex", gap: 8 }}>
                  <input value={newBtcAddress} onChange={e => setNewBtcAddress(e.target.value)} placeholder="bc1q..." style={{ flex: 1 }} />
                  <button className="btn-green" onClick={() => handleConfigUpdate("btcAddress", newBtcAddress)}>{configSaved === "btc" ? "Saved!" : "Update"}</button>
                </div>
              </div>
              <div className="field">
                <label>Monero (XMR) Address</label>
                <div style={{ display: "flex", gap: 8 }}>
                  <input value={newXmrAddress} onChange={e => setNewXmrAddress(e.target.value)} placeholder="4..." style={{ flex: 1 }} />
                  <button className="btn-green" onClick={() => handleConfigUpdate("xmrAddress", newXmrAddress)}>{configSaved === "xmr" ? "Saved!" : "Update"}</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="admin-table-wrap">
            ...
            <div style={{ padding: 24 }}>
              <div className="field">
                <label>Max Media Posts (absolute limit)</label>
                <div style={{ display: "flex", gap: 8 }}>
...
export default function App() {
  ...
  const handleSaveMedia = async (m) => {
    ...
    if (result && result.ok) {
      toast(`"${m.title}" media added`, "success");
      db.logActivity("created", "media", m.title);
      setActivityLog(await db.loadActivityLog());
    } else if (connStatus === "local") {
      toast(`"${m.title}" added (local only)`, "warn");
    } else {
  ...
  const handleUpdateSiteConfig = async (field, value) => {
    ...
    if (result && result.ok) {
      toast(`${field === "buyMeACoffee" ? "Coffee link" : field === "btcAddress" ? "BTC address" : field === "xmrAddress" ? "XMR address" : "Max media"} updated`, "success");
    } else if (connStatus === "local") {
