...
const db = {
  ...
  async loadSiteConfig() {
    const fallback = { heroSubtitle: DEFAULT_DONATE.heroSubtitle, maxMediaPosts: DEFAULT_DONATE.maxMediaPosts, donateMethods: JSON.parse(DEFAULT_DONATE.donateMethods) };
    if (!isSupabaseConfigured()) return fallback;
    const { data, error } = await supabase.from("site_config").select("*").eq("id", 1).single();
    if (error) {
      console.error("loadSiteConfig error:", error.message);
      // Table might not exist — try to create it
      if (error.message.includes("does not exist") || error.code === "PGRST116" || error.code === "42P01") {
        console.log("site_config table missing — run the SQL setup in Supabase");
      }
      return fallback;
    }
    if (!data) return fallback;
    console.log("Loaded site_config:", data);

    // Handle new schema (donate_methods as JSON string)
    if (data.donate_methods) {
      let methods;
      try { methods = JSON.parse(data.donate_methods); } catch { methods = JSON.parse(DEFAULT_DONATE.donateMethods); }
      return {
    ...
    // Handle old schema (buy_me_a_coffee, btc_address, xmr_address columns)
    if (data.buy_me_a_coffee || data.btc_address || data.xmr_address) {
      const methods = [];
      if (data.buy_me_a_coffee) methods.push({ id: "coffee", label: "Buy Me a Coffee ($)", type: "link", value: data.buy_me_a_coffee });
      if (data.btc_address) methods.push({ id: "btc", label: "Bitcoin (BTC)", type: "crypto", value: data.btc_address });
      if (data.xmr_address) methods.push({ id: "xmr", label: "Monero (XMR)", type: "crypto", value: data.xmr_address });
      return {
        heroSubtitle: data.hero_subtitle || DEFAULT_DONATE.heroSubtitle,
        maxMediaPosts: data.max_media_posts || DEFAULT_DONATE.maxMediaPosts,
        donateMethods: methods.length > 0 ? methods : JSON.parse(DEFAULT_DONATE.donateMethods),
      };
    }
  ...
  async updateSiteConfig(updates) {
    ...
    if (error) {
      ...
      // If donate_methods column doesn't exist, try old schema
      if (error.message && error.message.includes("donate_methods")) {
        console.log("Trying old schema columns...");
        const oldRow = { updated_at: new Date().toISOString() };
        if (updates.donateMethods) {
          const methods = updates.donateMethods;
          const coffee = methods.find(m => m.type === "link");
          const btc = methods.find(m => m.label.toLowerCase().includes("btc") || m.label.toLowerCase().includes("bitcoin"));
          const xmr = methods.find(m => m.label.toLowerCase().includes("xmr") || m.label.toLowerCase().includes("monero"));
          if (coffee) oldRow.buy_me_a_coffee = coffee.value;
          if (btc) oldRow.btc_address = btc.value;
          if (xmr) oldRow.xmr_address = xmr.value;
        }
        if (updates.maxMediaPosts !== undefined) oldRow.max_media_posts = updates.maxMediaPosts;
        const { error: err2 } = await supabase.from("site_config").update(oldRow).eq("id", 1);
        if (err2) { console.error("Old schema save also failed:", err2); return { ok: false, error: err2.message }; }
        return { ok: true };
      }
...
function AnalyticsChart({ data, range, onRangeChange, onReload }) {
      ...
      <div className="chart-container">
        <div className="chart-header">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {onReload && <button className="chart-range-btn" onClick={() => onReload(range)} title="Refresh">↻</button>}
          </div>
        </div>
